...
var _ = SIGDescribe("Cluster size autoscaling [Slow]", func() {
	...
	It("shouldn't add new node group if not needed [Feature:ClusterSizeAutoscalingScaleWithNAP2]", func() {
		framework.SkipUnlessProviderIs("gke")
		const extraPoolName = "extra-pool"
		addNodePool(extraPoolName, "n1-standard-2", 0)
		framework.ExpectNoError(enableAutoscaler(extraPoolName, 0, 2))
		defer deleteNodePool(extraPoolName)
		framework.ExpectNoError(enableAutoprovisioning(""))
		By("Create pods")
	...
	It("should create new node if there is no node for node selector [Feature:ClusterSizeAutoscalingScaleWithNAP]", func() {
		...
		framework.ExpectNoError(WaitForClusterSizeFunc(f.ClientSet,
			func(size int) bool { return size == nodeCount }, time.Second))
		By("Change resource limits")
		framework.ExpectNoError(enableAutoprovisioning("\"resource_limits\":{\"name\":\"cpu\", \"minimum\":2, \"maximum\":8}"))
		By("Wait for scale up")
		framework.ExpectNoError(WaitForClusterSizeFunc(f.ClientSet,
			func(size int) bool { return size == nodeCount+1 }, scaleUpTimeout))
		By("Check if NAP group was created")
...
func enableAutoprovisioning(resourceLimits string) error {
	...
	if resourceLimits != "" {
		...
		body = `{"update": {"desired_cluster_autoscaling": {"enable_node_autoprovisioning": true "resource_limits":{"name":"cpu", "minimum":200, "maximum":%d}, "resource_limits":{"name":"memory", "minimum":0, "maximum":10000000}}}}`
	}
